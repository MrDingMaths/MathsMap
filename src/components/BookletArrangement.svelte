<script>
 ...
 const runResize=(id,weights)=>onresize?onresize(id,weights):documentActions.resizeLayout(block.id,id,weights);
 const dropPosition=(e,n)=>{const y=(e.clientY-e.currentTarget.getBoundingClientRect().top)/e.currentTarget.clientHeight;return n.type==='group'&&y>.25&&y<.75?'inside':y<.5?'before':'after';};
 ...
 function start(event,n,index){
  ...
  const finish=e=>{window.removeEventListener('pointermove',move);window.removeEventListener('pointerup',finish);window.removeEventListener('pointercancel',cancel);window.removeEventListener('keydown',escape);el.style.gridTemplateColumns=old;if(e?.type==='pointerup')runResize(n.id,weights);};
 ...
 function style(n){const indent=resolved.labelIndents.get(n.id)??0;return `--arr-vertical:${{top:'start',middle:'center',bottom:'end'}[n.verticalAlign]??'start'};--arr-gap:${n.gap??2}mm;${n.type==='group'?`--arr-label-left:${indent}mm;`:''}margin-top:${n.before??0}mm;margin-bottom:${n.after??0}mm;${indent||n.inset!=null?`padding-left:${(n.inset??7)+indent}mm;`:''}${n.minHeight?`min-height:${n.minHeight}mm;`:""}${n.width?`width:${n.width}mm;`:''}${n.keepInline?'white-space:nowrap;':''}${n.align==='center'?'margin-inline:auto;':n.align==='right'?'margin-left:auto;':''}${n.keepTogether===false?'break-inside:auto;':''}`;}
 ...
 function resizeDiagramHeight(event,n){
  ...
  const finish=e=>{window.removeEventListener('pointermove',move);window.removeEventListener('pointerup',finish);window.removeEventListener('pointercancel',cancel);window.removeEventListener('keydown',escape);diagram.style.width=original;if(e?.type==='pointerup')onmeasure?.(n.id,{width:Math.round(width*10)/10});};
...
{#snippet renderNode(n,depth=0)}
 ...
 {#if n.type==='group'}
  ...
  {#if canResize&&n.direction==='row'}{#each n.children.slice(0,-1) as child,i}<button class="column-handle" onclick={e=>e.stopPropagation()} style:left={n.children.slice(0,i+1).reduce((a,c)=>a+(c.weight??1),0)/n.children.reduce((a,c)=>a+(c.weight??1),0)*100+'%'} aria-label={'Resize column boundary '+(i+1)} onpointerdown={e=>start(e,n,i)} onkeydown={e=>{if(['ArrowLeft','ArrowRight'].includes(e.key)){e.preventDefault();e.stopPropagation();const w=n.children.map(c=>c.weight??1),d=e.key==='ArrowLeft'?-.1:.1;if(w[i]+d>.1&&w[i+1]-d>.1){w[i]+=d;w[i+1]-=d;runResize(n.id,w);}}}}>↔</button>{/each}{/if}
...
<style>
 ...
 .space-handle,.width-handle{position:absolute;z-index:5;font:12px system-ui;padding:0;min-height:18px;border:1px solid var(--booklet-blue);background:white;color:var(--booklet-blue);touch-action:none}.space-handle{bottom:-9px;left:45%;width:30px;cursor:ns-resize}.width-handle{right:-10px;top:40%;width:20px;height:30px;cursor:ew-resize}
 .arr-group:not(.arr-row){justify-content:var(--arr-vertical,start)}
