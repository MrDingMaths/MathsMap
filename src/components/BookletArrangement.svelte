<script>
 ...
 function style(n){const indent=resolved.labelIndents.get(n.id)??0;return `--arr-gap:${n.gap??2}mm;${n.type==='group'?`--arr-label-left:${indent}mm;`:''}margin-top:${n.before??0}mm;margin-bottom:${n.after??0}mm;${indent||n.inset!=null?`padding-left:${(n.inset??7)+indent}mm;`:''}${n.width?`width:${n.width}mm;`:''}${n.keepInline?'white-space:nowrap;':''}${n.align==='center'?'margin-inline:auto;':n.align==='right'?'margin-left:auto;':''}${n.keepTogether===false?'break-inside:auto;':''}`;}
