<script>
 ...
 function start(event,n,index){
  ...
  const finish=e=>{window.removeEventListener('pointermove',move);window.removeEventListener('pointerup',finish);window.removeEventListener('pointercancel',cancel);window.removeEventListener('keydown',escape);el.style.gridTemplateColumns=old;if(e?.type==='pointerup')runResize(n.id,n.children.map(c=>weights[children.indexOf(c)]??c.weight??1));};
...
<style>
 .internal-rules.arr-row>:global([data-arrangement-id]+[data-arrangement-id])::after{content:"";position:absolute;left:calc(var(--arr-gap) / -2);top:0;bottom:0;border-left:.2mm solid #cccccc;pointer-events:none}
