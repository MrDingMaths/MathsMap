<script>
  ...
  import { groupBookletBlocks, investigationDescription, resolvePreviewAssets, visibleImportedQuestionTitle } from '../lib/booklet-preview.js';
...
{#snippet blockBody(block, index = 0, insideAtom = false)}
  ...
  {:else if block.type === 'worked-example'}
    <section class:inside-atom={insideAtom} class="theory-section">
      ...
      <div class:body-box={!insideAtom} class="example">
  ...
  {:else if block.type === 'callout'}
    {#if insideAtom}
      <EditableBookletText value={block.content} rootId={block.id} pointer="/content" fillCloze={showTheorySolutions} {...editProps()} edited={isEdited(block.id, '/content')} />
    {:else}
      <section class="theory-section"><BookletSectionHeader kind={calloutKind(block)} label={calloutLabel(block)} subtitle={calloutDescription(block)} editMode={editMode} rootId={block.id} pointer="/title" {onContentEdit} {onContentRevert} {onEditingChange} {isEdited} /><div class="body-box"><EditableBookletText value={block.content} rootId={block.id} pointer="/content" fillCloze={showTheorySolutions} {...editProps()} edited={isEdited(block.id, '/content')} /></div></section>
    {/if}
  ...
  <div class="preview-page">
    ...
    {:else}
      <article class="booklet-page" data-page-number={page.pageNumber}>
        <header class="section-band"><EditableBookletText value={page.section?.title ?? ''} rootId={page.id} pointer="/section/title" {...editProps()} edited={isEdited(page.id, '/section/title')} /></header>
        <main>
