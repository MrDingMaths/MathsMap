<script>
  ...
  import {logicalUnits,flowNumbers,flowCommand,captureFlowClipboard,flowId} from '../lib/booklet-flow.js';
...
<div class="flow-outline" inert={disabled}>
  ...
  {#each project.topics as topic (topic.id)}
    <details open class="topic"><summary>{topic.title}</summary>
      ...
      {#each project.sections.filter(s=>s.topicId===topic.id) as section (section.id)}
        <details open class="section" ondragover={e=>e.preventDefault()} ondrop={e=>{e.preventDefault();e.stopPropagation();drop(e.dataTransfer.getData('application/x-booklet-block'),section.id,null);}}>
          <summary onclick={()=>{destination=section.id;onsection?.(section.id);}}>{section.title}</summary>
          <details><summary>Page boundary</summary>
            ...
            {#if section.phase==='practice'}<label><input type="checkbox" checked={section.showDifficultyHeading!==false} onchange={e=>patchSection(section.id,{showDifficultyHeading:e.currentTarget.checked})}/>Show difficulty heading</label>{/if}
          </details>
          {#if section.phase==='practice'}<details><summary>Question numbering</summary><label>Start numbering at<input type="number" min="1" step="1" placeholder="Continue numbering" value={section.numberingStart??''} onchange={e=>patchSection(section.id,{numberingStart:e.currentTarget.value?Math.max(1,Math.trunc(Number(e.currentTarget.value))):null})}/></label></details>{/if}
          <details><summary>Section settings</summary><label>Section title<input value={section.title} onchange={e=>patchSection(section.id,{title:e.currentTarget.value||section.title})}/></label><label>Topic<select value={section.topicId} onchange={e=>reassignSection(section.id,e.currentTarget.value)}>{#each project.topics as t}<option value={t.id}>{t.title}</option>{/each}</select></label><label>Phase<select value={section.phase} onchange={e=>patchSection(section.id,{phase:e.currentTarget.value,role:e.currentTarget.value})}><option value="teaching">Teaching</option><option value="practice">Practice</option><option value="front-matter">Front matter</option></select></label><label>Difficulty set<select value={section.difficulty??''} onchange={e=>patchSection(section.id,{difficulty:e.currentTarget.value||null,title:e.currentTarget.value||'Practice'})}><option value="">No tier</option>{#each ['Foundation','Development','Mastery','Challenge'] as tier}<option>{tier}</option>{/each}</select></label><button onclick={()=>moveSection(section.id,-1)}>Section up</button><button onclick={()=>moveSection(section.id,1)}>Section down</button><button onclick={()=>joinPrevious(section.id)}>Join previous section</button></details>
