<script>
  ...
  let { diagram, initialDraft=null, ondraft=()=>{}, onsave=()=>{}, focused=false, sourceUrl='', colourMode='original', assetBase='' }=$props();
  ...
  function restore(v){model=v.mathematicalModel??null;code=v.code;width=v.width;crop=v.sourceRegion;colour=v.colourMode;error='';ondraft(getValue());}
  ...
  export function getValue(){return {code,width,sourceRegion:crop,colourMode:colour,mathematicalModel:model?copy(model):null,valid:!error};}
  ...
  function cancel(){model=null;code=diagram.code??diagram.src??'';width=diagram.widthMm??78;crop=diagram.sourceRegion??null;colour=colourMode;error='';ondraft(null);}
</script>
