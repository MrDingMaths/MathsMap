<script>
  ...
  let report=$state(null),error=$state(''),limit=$state(30);
  $effect(()=>{
    const snapshot=project;let active=true;report=null;
...
<details class="coverage"><summary>Source coverage and exceptions</summary>
  {#if error}<p role="alert">{error}</p>{:else if !report}<p>Checking current content…</p>{:else}
