...
export function captureEditorSelection(editor) {
  ...
  if(active?.matches('math-field')&&surface.contains(active))return {mathIndex:[...surface.querySelectorAll('math-field')].indexOf(active),position:active.position};
...
function focusMathField(field, surface, position) {
  const focus=()=>{field.focus({preventScroll:true});if(position!=null)field.position=position;};
  focus();
...
export function runEditorCommand(editor, name, value) {
  ...
  else if(name==='colour'){const mf=document.activeElement?.closest('math-field');if(mf&&c.surface.contains(mf)){mf.applyStyle({color:value});}else{c.restoreRange();document.execCommand('foreColor',false,value);c.surface.querySelectorAll('font[color]').forEach(n=>{n.dataset.colour=n.getAttribute('color');});}c.capture();c.remember();c.emit();}
