...
import {standardTikzColours,stripStandardTikzPalette} from '../../public/libs/maths-editor/booklet-palette.mjs';
...
export function readGraphModel(code) {
 code=graphSourceWithoutColourMetadata(code);
 const match=String(code??'').match(/^% mathsmap-graph-model: (.+)$/m);
 if(!match)return null;
 try {const model=JSON.parse(match[1]);const strip=s=>stripStandardTikzPalette(standardTikzColours(s));return strip(graphTikz(model))===strip(code)||strip(graphTikz(model,{legacyStrokes:true}))===strip(code)?model:null;} catch{return null;}
}
