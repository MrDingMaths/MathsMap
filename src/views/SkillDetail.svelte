<script>
  ...
  let level = $state('none');
  let locked = $state([]);
  $effect(() => subscribe(() => {
    level = skill ? getMastery(skill.id) : 'none';
    ...
    <div class="skill-grid">
      ...
      <div>
        <div class="title-block">
          ...
          <div class="row">
            ...
            {/each}
            <a class="map-link" href={href(`/map?skill=${skill.id}${courseId ? `&course=${courseId}` : ''}`)}>View on map →</a>
          </div>
        </div>

        <!-- status banner -->
        <div class="status m-{level}">
          <span class="status-dot"></span>
          <span>{MASTERY[level].verb}</span>
        </div>
        ...
        <section>
          <div class="sec-head">
            <span class="sec-dot mastered"></span>Unlocks
      ...
      <div class="skill-side">
        ...
        <div class="card meta">
          ...
          <div class="meta-row"><span class="meta-dot mastered"></span><span class="meta-label">Unlocks</span><span class="meta-val">{unlocks.length}</span></div>
        </div>
      </div>
    </div>
  {:else}
...
<style>
  ...
  .status.m-learning { background: color-mix(in srgb, var(--m-learning) 14%, transparent); color: var(--m-learning); }
  .status.m-proficient { background: color-mix(in srgb, var(--m-proficient) 14%, transparent); color: var(--m-proficient); }
  .status.m-mastered { background: color-mix(in srgb, var(--m-mastered) 14%, transparent); color: var(--m-mastered); }
  .status.m-learning .status-dot { background: var(--m-learning); }
  .status.m-proficient .status-dot { background: var(--m-proficient); }
  .status.m-mastered .status-dot { background: var(--m-mastered); }
