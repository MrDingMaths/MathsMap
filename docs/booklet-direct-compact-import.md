...
For source-specific geometry colours outside the shared graph palette, retain occurrence-scoped evidence in the native SVG metadata, for example `data-graph-source-palette="4654B5" data-graph-source-reference="PDF page 8, example triangle"`. Use six-digit hexadecimal colours without `#` in TikZ raw specials, because TeX can double that character. The layout checker permits only the declared colours for that diagram; a nonempty source reference is required. This does not certify colour fidelity or bypass geometry, stroke, typography or visual-review checks.
