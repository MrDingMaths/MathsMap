...
General diagram lines and labels use solid black `#000000`, overriding decorative source palettes. Preserve semantic colours with explicit roles and occurrence-specific reasons, using the [diagram colour contract](booklet-diagram-colours.md). Graph curves, axes, grids and legends retain their own rules. Historical `data-graph-source-palette` metadata remains original evidence; it no longer exempts ordinary geometry outlines from black. Use six-digit hex without `#` in TikZ raw specials. Palette, content, geometry and final-size visual checks remain separate.
...
Semantic page execution uses bounded parallelism: `node scripts/booklet/semantic-packets.mjs inventory --run-id RUN --pages 1-N --config CONFIG.json --concurrency 3`, followed by mathematical review, representative acceptance and the equivalent `author` batches. New runs default to three workers; an explicit CLI value or the saved manifest value is respected. Inventory pages may run in parallel; author pages proceed after their applicable review-first gates pass. Pending pages do not prevent unaffected pages from running. Both stages receive teaching context. Every successful page records its actual execution configuration, input hash, output hash and usage metrics. Matching results resume automatically, including completed pages within an explicit attempt. Legacy caches without output hashes, edited results and changed inputs require inspection and a new immutable `--attempt`. Failed attempts preserve the earlier canonical result; an edit made during a call prevents its replacement. Cache hits establish reusable generation, not source acceptance.

Author calls use explicit compact reconstruction, schema and solution contracts in `scripts/booklet/token-efficient-prompts.mjs`. Common contracts and shared teaching context precede page-specific diagram rules and evidence, allowing a reusable prefix without assuming a provider cache hit. Drawing/sketching requests receive diagram guidance even without an existing source figure. Source palette evidence is retained while rendered colours use the standard palette; cover content is evidence-dependent; page 1 is not automatically discarded.

All configured teaching text and image paths remain available. At most two teaching images are attached as previews by default, plus the target page and explicit supplemental images, deduplicated by path. Workers must inspect later referenced images when the taught method requires them. Config `teachingImageLimit` changes the preview count; `pageTeachingPages: {"10": [6,8]}` can explicitly select teaching context for a page. Word retrieval defaults to two 2,400-character windows expanded to complete lines, with overlaps merged and original whitespace/offsets preserved. Config `wordRetrieval: {windowChars: 5000, count: 2}` requests broader excerpts. Full Word evidence stays linked for missing ranges and images; uncertain evidence remains a finding. These limits must never cause omission of source content or required methods.

Add `--dry-run` to inspect prompt character counts by section, attached image counts, input hashes and cache status without calls or writes. `benchmark-semantic-prompts.mjs --run-id RUN --pages RANGE --config CONFIG.json` compares complete assembled author prompts against commit `020d8a64`, offline. See [the workflow review](booklet-transcription-efficiency-review.md) for measured scope and limitations. Source comparison, reconciliation, rendering and acceptance remain separate required checks.
...
That interface remains compatible. Review-first runs instead store the complete approved patch bundle once in `workflow/issues.json`; author prompts, assembly and project propagation consume its effective outputs and retain correction references. See the review-first commands for exact-original conflicts, current status/history separation, bank-safe saves and invalidated acceptance. Do not maintain parallel handwritten correction lists or copy old pending prose into current progress.
...
Preserve original source-colour evidence separately. Render every editable occurrence with the [standard booklet palette](booklet-standard-palette.md), including Index Laws source red `#AA0505` mapped to `#ef6068`. Presentation requirements check mathematical colour roles through that palette; source hex values cannot authorise nonstandard output.
...
The user accepted the direct-to-flexible workflow and authorised retirement on 9 September 2026. Projects is now the sole booklet workspace. Historical run listing, detail browsing, review writes and materialization are removed. Old import links redirect to Projects without creation. Explicit candidate imports, source-file serving, correction conflict checks and exact-mode rendering remain. Historical reports and snapshots are retained in ignored local recovery storage; required regression inputs remain in test fixtures.
