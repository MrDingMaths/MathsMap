...
Semantic page execution uses bounded parallelism: `node scripts/booklet/semantic-packets.mjs inventory --run-id RUN --pages 1-N --config CONFIG.json --concurrency 3`, followed by the equivalent `author` command. New runs default to three workers; an explicit CLI value or the saved manifest value is respected. Inventory pages may run in parallel, and author pages may run in parallel after their inventories exist. Both stages receive teaching context. Every successful page records its actual execution configuration, input hash, output hash and usage metrics. Matching results resume automatically, including completed pages within an explicit attempt. Legacy caches without output hashes, edited results and changed inputs require inspection and a new immutable `--attempt`. Failed attempts preserve the earlier canonical result; an edit made during a call prevents its replacement. Cache hits establish reusable generation, not source acceptance.
