...
Semantic page execution uses bounded parallelism: `node scripts/booklet/semantic-packets.mjs inventory --run-id RUN --pages 1-N --config CONFIG.json --concurrency 3`, followed by the equivalent `author` command. The default is three workers; lower it when the provider reports throttling. Inventory pages may run in parallel, and author pages may run in parallel after their inventories exist. Each successful page records a hash of its source, prompt, model and evidence inputs. Unchanged pages are resumed automatically. A changed cached page must be rerun under a new immutable `--attempt` so an older result is never silently overwritten. Shared teaching context is cached per topic/page under the local run directory.

Author calls use a compact page-level contract. The reusable reconstruction rules are kept in `scripts/booklet/token-efficient-prompts.mjs`, while TikZ rules are added only when the independent inventory identifies a genuine visual. This reduces repeated prompt tokens and keeps shared teaching context in a stable prefix for provider-side caching; it does not relax source comparison, reconciliation, rendering, or acceptance checks.
