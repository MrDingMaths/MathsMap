...
- Production `scripts/booklet/codex-transcription.mjs::runTranscriptionTasks` appends that prompt through `src/lib/booklet-house-style.js`. Fresh reconstruction and dedicated diagram stages also receive it through `scripts/booklet/benchmark.mjs::reconstructionPrompt`. The active files are `scripts/booklet/prompts/reconstruction-v4.md` and `scripts/booklet/prompts/diagram-reconstruction-v4.md`; both state the rule explicitly. Historical pinned inputs remain unchanged.
