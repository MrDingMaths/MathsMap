...
export function normalizeDocument(raw = {}) {
  ...
  const blocks = (items, depth = 0) => {
    ...
    return (items ?? []).map(rawNode => {
      ...
      if (n.type === 'layout') return { ...base, arrangement:choice(n.arrangement,['investigation','parallel','worked-rows','scaffold','cards'],'parallel'), title:String(n.title ?? ''),border:n.border!==false,padding:limit(n.padding,3,0,20),margin:limit(n.margin,3,0,20),gap:limit(n.gap,3,0,20), columns:Math.round(limit(n.columns,2,1,6)), ...(n.tracks?{tracks:n.tracks.map(v=>limit(v,1,.01,1000))}:{}), slots:(n.slots ?? []).map(slot => ({ id:id(slot.id), blocks:blocks(slot.blocks,depth+1) })) };
...
export function renderDocument(raw, { math = latex => escapeHtml(latex), editable = false, fillCloze = false, annotationMath = math } = {}) {
  ...
  const inline = n => n.type === 'tab' ? '<span data-tab contenteditable="false" aria-label="Tab" style="display:inline-block;width:10mm;white-space:nowrap;vertical-align:baseline;overflow:hidden">&#8203;</span>' : n.type === 'inline-image' ? renderInlineImage(n,editable) : n.type === 'text' ? n.marks.reduce((s,m) => `<${{bold:'strong',italic:'em',underline:'u'}[m]}>${s}</${{bold:'strong',italic:'em',underline:'u'}[m]}>`, e(n.text)) : n.type === 'break' ? '<br>' : n.type === 'cloze' ? renderCloze(n) : `${mathCaret}<span data-math="true" data-display="${n.display}" contenteditable="false" style="${n.display ? 'display:block;' : 'display:inline-block;vertical-align:baseline;'}${/\\(?:d?frac|tfrac|cfrac)\b/.test(n.latex)?'padding-block:.15em;':''}">${editable ? `<math-field>${e(n.latex)}</math-field>` : math(n.latex,n.display)}</span>${mathCaret}`;
