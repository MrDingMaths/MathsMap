...
import { clozeWidthMm, clozeLayout, BOOKLET_HOUSE_STYLE } from './house-style.mjs';
...
export const safeLink=value=>/^(https?:\/\/|mailto:|#)/i.test(String(value??''))?String(value):'';
// Printed resource URLs stay editable as ordinary text and become usable links
// in preview/PDF. Never interpret arbitrary HTML or non-web protocols.
export function linkedTextHtml(value) {
  return String(value ?? '').split(/(https?:\/\/[^\s<>"']+)/gi).map((part,i) => {
    if (!(i % 2)) return escapeHtml(part);
    const url=part.replace(/[.,;!?]+$/,''),tail=part.slice(url.length);
    return `<a href="${escapeHtml(url)}" style="color:inherit;text-decoration:underline">${escapeHtml(url)}</a>${escapeHtml(tail)}`;
  }).join('');
}
...
export function renderDocument(raw, { math = latex => escapeHtml(latex), editable = false, fillCloze = false, annotationMath = math, editableMathPreview = false } = {}) {
  ...
  const plainInline = n => n.type === 'tab' ? '<span data-tab contenteditable="false" aria-label="Tab" style="display:inline-block;width:10mm;white-space:nowrap;vertical-align:baseline;overflow:hidden">&#8203;</span>'+mathCaret : n.type === 'inline-image' ? renderInlineImage(n,editable) : n.type === 'text' ? n.marks.reduce((s,m) => `<${{bold:'strong',italic:'em',underline:'u'}[m]}>${s}</${{bold:'strong',italic:'em',underline:'u'}[m]}>`, n.href?'<a href="'+e(n.href)+'" style="color:inherit;text-decoration:underline">'+e(n.text)+'</a>':editable ? e(n.text) : linkedTextHtml(n.text)) : n.type === 'break' ? '<br>' : n.type === 'cloze' ? renderCloze(n) : `${mathCaret}<span data-math="true"${n.semanticRole==='correctness-marker'?' data-semantic-role="correctness-marker"':''} data-display="${n.display}"${editable&&editableMathPreview?' data-math-preview-host':''} contenteditable="false" style="${n.semanticRole==='correctness-marker'?`font-size:${BOOKLET_HOUSE_STYLE.teaching.correctnessMarkerPt}pt;`:''}${n.display ? 'display:block;' : 'display:inline-block;vertical-align:baseline;'}${/\\(?:d?frac|tfrac|cfrac)\b/.test(n.latex)?'padding-block:.15em;':''}">${editable ? `${editableMathPreview?`<span data-math-preview aria-hidden="true">${math(n.latex,n.display)}</span>`:''}<math-field>${e(spaceFractionSteps(n.latex))}</math-field>` : math(n.latex,n.display)}</span>${mathCaret}`;
...
export function exportSource(raw) {
  ...
  visitDocument(doc,n=>{if(n.inlines?.some(i=>i.href))losses.push({id:n.id,type:'link',message:'Plain source loses link destinations; use structured JSON or rich clipboard.'});if(n.inlines?.some(i=>i.colour))losses.push({id:n.id,type:'colour',message:'Plain source loses text and equation colours; use structured JSON or rich clipboard.'});if(n.type==='paragraph'&&(n.tabStops?.length||n.inlines.some(i=>i.type==='tab')))losses.push({id:n.id,type:'tabs',message:'Plain text retains tab characters but loses tab positions, alignment and leaders.'});if(['inline-image','image','table','layout','spacer','annotated-equation'].includes(n.type))losses.push({id:n.id,type:n.type,message:'Plain source is a lossy projection. Use structured JSON or rich clipboard for lossless transfer.'});});
