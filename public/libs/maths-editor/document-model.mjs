...
export function renderDocument(raw, { math = latex => escapeHtml(latex), editable = false, fillCloze = false, annotationMath = math } = {}) {
  ...
  const mathCaret = editable ? '<span data-math-caret>\u200b</span>' : '';
  const plainInline = n => n.type === 'tab' ? '<span data-tab contenteditable="false" aria-label="Tab" style="display:inline-block;width:10mm;white-space:nowrap;vertical-align:baseline;overflow:hidden">&#8203;</span>' : n.type === 'inline-image' ? renderInlineImage(n,editable) : n.type === 'text' ? n.marks.reduce((s,m) => `<${{bold:'strong',italic:'em',underline:'u'}[m]}>${s}</${{bold:'strong',italic:'em',underline:'u'}[m]}>`, e(n.text)) : n.type === 'break' ? '<br>' : n.type === 'cloze' ? renderCloze(n) : `${mathCaret}<span data-math="true" data-display="${n.display}" contenteditable="false" style="${n.display ? 'display:block;' : 'display:inline-block;vertical-align:baseline;'}${/\\(?:d?frac|tfrac|cfrac)\b/.test(n.latex)?'padding-block:.15em;':''}">${editable ? `<math-field>${e(spaceFractionSteps(n.latex))}</math-field>` : math(n.latex,n.display)}</span>${mathCaret}`;
  ...
  const render = (nodes, compactImages = false) => nodes.map(n => {
    ...
    if(n.type === 'paragraph') return `<p ${attr} ${n.tabStops?`data-tab-stops="${e(JSON.stringify(n.tabStops))}"`:""} style="${n.fontSize?`font-size:${n.fontSize}pt;`:''}text-align:${n.align};margin:${n.spaceBefore}mm 0 ${n.spaceAfter}mm;padding-left:${n.indent}mm;line-height:${n.lineHeight}">${n.inlines.map(inline).join('') || '<br>'}</p>`;
    if(n.type === 'spacer') return `<div ${attr} contenteditable="false" style="height:${n.height}mm">${editable ? 'Working space' : ''}</div>`;
    if(n.type === 'image') { const [t,r,b,l] = n.crop, floating=n.align.startsWith('beside-'); return `<figure ${attr} contenteditable="false" style="width:${n.width}mm;max-width:100%;${floating?'float:'+n.align.slice(7)+';':''}display:${n.align === 'inline' ? 'inline-block' : 'block'};margin:${compactImages?0:2}mm ${floating?'3mm':n.align === 'left' || n.align === 'inline' ? 'auto 2mm 0' : n.align === 'right' ? '0 2mm auto' : 'auto'}"><div style="overflow:hidden;position:relative;aspect-ratio:${n.aspectRatio*(100-l-r)/(100-t-b)}"><img src="${e(n.src)}" alt="${e(n.alt)}" style="position:absolute;max-width:none;width:${10000/(100-l-r)}%;left:${-100*l/(100-l-r)}%;top:${-100*t/(100-t-b)}%"></div><figcaption>${e(n.caption)}</figcaption></figure>`; }
    if(n.type === 'table') {
    ...
    if(n.arrangement==='cards'&&n.slots.some(s=>s.label!=null))return `<section ${attr} contenteditable="false" class="me-layout me-cards" style="width:${n.widthMm??170}mm;max-width:100%;margin:${n.margin}mm ${n.align==='left'?'auto '+n.margin+'mm 0':n.align==='right'?'0 '+n.margin+'mm auto':'auto'}"><div style="display:grid;grid-template-columns:repeat(${n.columns},minmax(0,1fr));gap:${n.gap}mm">${n.slots.map(s=>`<div data-slot="${e(s.id)}" style="display:flex;align-items:center;gap:3mm;min-width:0"><span data-card-label style="width:4mm;flex:none;font-weight:700">${e(s.label??'')}</span><div data-card-face style="width:${s.widthMm??30}mm;max-width:calc(100% - 7mm);box-sizing:border-box;border:.2mm solid #d3d7db;box-shadow:1.5mm 1.5mm #ddd;padding:2mm;text-align:center" ${editable?'contenteditable="true"':''}>${render(s.blocks)}</div></div>`).join('')}</div></section>`;
    return `<section ${attr} contenteditable="false" class="me-layout me-${n.arrangement}" style="margin:${n.margin}mm 0;padding:${n.padding}mm;border:${n.border?'1px solid #becbdd':'0'}">${n.title?`<strong>${e(n.title)}</strong>`:''}<div style="display:grid;grid-template-columns:${n.arrangement==='worked-rows'?'minmax(0,1fr)':Array.from({length:n.columns},(_,i)=>`minmax(0,${n.tracks?.[i]??1}fr)`).join(' ')};gap:${n.gap}mm">${n.slots.map(s => `<div data-slot="${e(s.id)}" style="${n.arrangement==='cards'?'border:.25mm solid #9eabb9;box-shadow:1mm 1mm #ddd;padding:2mm;text-align:center':''}" ${editable?'contenteditable="true"':''}>${render(s.blocks)}</div>`).join('')}</div></section>`;
  }).join('');
