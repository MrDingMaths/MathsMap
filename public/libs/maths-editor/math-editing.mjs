...
export function installMathEditing(editor) {
 ...
 const style=(value)=>{const mf=field();if(!mf||host.readonly)return;mf.focus();mf.applyStyle(value,{operation:'toggle'});commit();};
 editor.formatMath=mark=>{const mf=field();if(!mf||!bar.contains(document.activeElement)&&document.activeElement!==mf&&!editor.toolbar.contains(document.activeElement))return false;if(mark==='underline'){mf.executeCommand(['insert','\\underline{#0}']);commit();}else style(mark==='bold'?{variantStyle:'bold'}:{variantStyle:'italic'});return true;};
 const palette=()=>{const mf=field(),api=window.MathsEditor?.Palette;if(!mf||!api||host.readonly)return;api.open(mf,{onInsert:latex=>{if(!mf.isConnected)return;mf.executeCommand(['insert',latex]);commit();},onClose:()=>{if(mf.isConnected)mf.focus();}});};
 ...
 button('Edit LaTeX',()=>{const mf=field();if(!mf||host.readonly)return;dialog?.remove();dialog=document.createElement('dialog');dialog.className='me-latex-dialog';dialog.setAttribute('aria-label','Edit equation LaTeX');const label=document.createElement('label');label.textContent='Equation LaTeX';const input=document.createElement('textarea');input.setAttribute('aria-label','Equation LaTeX');input.value=mf.getValue('latex');label.append(input);const preview=document.createElement('div');preview.className='me-latex-preview';const update=()=>{preview.innerHTML=window.MathLive.convertLatexToMarkup(input.value);};input.oninput=update;update();dialog.append(label,preview);editor.button(dialog,'Apply equation',()=>{if(mf.isConnected){mf.setValue(input.value,{silenceNotifications:true});commit();}dialog.close();});editor.button(dialog,'Cancel',()=>dialog.close());dialog.addEventListener('close',()=>{dialog.remove();dialog=null;if(mf.isConnected)mf.focus();});host.append(dialog);dialog.addEventListener('keydown',e=>e.stopPropagation());dialog.showModal();input.focus();});
 ...
 const focus=e=>{if(e.target.matches?.('math-field')){active=e.target;bar.hidden=false;}else if(surface.contains(e.target)){active=null;bar.hidden=true;}};surface.addEventListener('focusin',focus);
 ...
 const place=(node,offset)=>{surface.focus();const range=document.createRange();range.setStart(node,offset);range.collapse(true);const selection=getSelection();selection.removeAllRanges();selection.addRange(range);editor.saveRange();active=null;bar.hidden=true;};
 ...
 return ()=>{host.removeEventListener('keydown',key,true);surface.removeEventListener('focusin',focus);surface.removeEventListener('move-out',move);dialog?.remove();bar.remove();if(active&&window.MathsEditor?.Palette?.isOpen())window.MathsEditor.Palette.close('editor-closed');};
}
