...
export const documentLayoutTools={
 ...
 mountLayoutPanel(panel,{category='all'}={}){
  let selectedRow=0,tableId='',selectionId='';
  const refresh=()=>{
   panel.replaceChildren();const at=locateDocumentNode(this.doc,this.selectedId);if(!at)return;
   const title=document.createElement('strong');title.textContent='Selected '+at.node.type;if(category==='all')panel.append(title);
   ...
   if(table&&category!=='arrange'){
    ...
    if(tableId!==table.id){tableId=table.id;selectedRow=0;}
    if(selectionId!==this.selectedId){selectionId=this.selectedId;const cell=at.ancestors.find(n=>n.type==='cell');const row=table.rows.findIndex(r=>r.some(c=>c.id===(cell?.id??at.node.id)));if(row>=0)selectedRow=row;}
    selectedRow=Math.min(selectedRow,Math.max(0,table.rows.length-1));
    const rowLabel=document.createElement('label'),rowSelect=document.createElement('select');rowLabel.textContent='Table row';rowSelect.setAttribute('aria-label','Table row');table.rows.forEach((row,i)=>rowSelect.append(new Option('Row '+(i+1),i)));rowSelect.value=selectedRow;rowSelect.onchange=()=>{selectedRow=Number(rowSelect.value);refresh();};rowLabel.append(rowSelect);panel.append(rowLabel);
    if(table.rows.length)field(`Table row ${selectedRow+1} minimum height (mm)`,table.rowHeights?.[selectedRow]??4,(n,v)=>{n.rowHeights??=[];n.rowHeights[selectedRow]=Math.max(4,v);});
   }
   if(category!=='layout'){
   ...
   if(category!=='layout')this.button(panel,at.node.type==='paragraph'?'Delete paragraph':'Delete selected block',()=>this.deleteNode());
  };
