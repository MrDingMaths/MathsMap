...
import { studioProject, editFields, splitTheory, mergeTheory, splitQuestionParts, mergeQuestionParts, mergeQuestionContinuation } from '../src/lib/booklet-review-model.js';
...
import { candidateValid } from '../src/lib/booklet-assembly.js';
...
test('direct field edits preserve the undo snapshot and never create proposals or approvals',()=>{
 const original=base(),snapshot=JSON.stringify(original);
 const edited=editFields(original,[{targetId:'a',path:'/content',after:'Changed wording'}]);
 assert.equal(edited.sections[0].blocks[0].content,'Changed wording');
 assert.equal(JSON.stringify(original),snapshot);
 assert.equal(edited.studio.proposals,undefined);
 assert.equal(edited.studio.approvals,undefined);
 assert.throws(()=>editFields(original,[{targetId:'a',path:'/content',after:'Partial edit'},{targetId:'b',path:'/id',after:'new-id'}]),/Protected/);
 assert.equal(JSON.stringify(original),snapshot,'a failed multi-field edit must not partially mutate the project');
});

test('assembly eligibility uses content and mappings rather than historical review decisions',()=>{
 const c=candidate('human','skill-a');
 c.reviewed=false;c.question.review.flags=['Check this wording'];
 assert.equal(candidateValid(c),true);
 c.question.content.children[0].prompt='$x/2$';
 assert.equal(candidateValid(c),false);
 c.question.content.children[0].prompt='Part a';c.skillIds=[];
 assert.equal(candidateValid(c),false);
});
...
test('theory split and merge retain content and source lineage',()=>{
 let p=base();p=splitTheory(p,'a');assert.equal(p.sections[0].blocks.length,3);
 const first=p.sections[0].blocks[0].id;assert.deepEqual(p.studio.lineage[first].sourceIds,['a']);p=mergeTheory(p,first);assert.equal(p.sections[0].blocks[0].content,'First atom.\n\nSecond atom.');
});
...
function candidate(id,skill,tier='Foundation',origin='mathsmap') {return {question:normaliseQuestion({id,classification:{primarySkillId:skill,difficulty:tier,difficultyReason:'Fixture demand'},content:{id:id+'-root',prompt:'Shared stem',children:[{id:id+'-a',prompt:'Part a',answer:{short:'1',worked:'1'}},{id:id+'-b',prompt:'Hence part b',answer:{short:'2',worked:'2'}}]}}),origin,skillIds:[skill],archetype:'routine',tier,prerequisiteIds:[],reviewed:true};}
...
test('part grouping retains the shared stem, diagrams, labels, answers and dependencies',()=>{
 ...
 const original=structuredClone(q.content);p=splitQuestionParts(p,q.content.id,1);
 ...
 p=mergeQuestionParts(p,p.sections[0].blocks[0].content.children[0].id);assert.deepEqual(allNodes(p.sections[0].blocks[0].content).filter(n=>!n.children?.length),original.children);
});
...
test('assembly rekeys overlays and part dependencies together and pins source revisions',()=>{
 ...
 const c=candidate('original','skill-a');c.revision='reviewed-sha';c.question.content.questionDiagrams=[{id:'base',role:'question',format:'tikz',code:'base'}];c.question.content.children[0].answer.solutionDiagrams=[{id:'overlay',role:'solution-overlay',transparent:true,overlayOf:'base',format:'tikz',code:'overlay'}];c.question.content.children[1].dependsOn=[c.question.content.children[0].id];
...
test('cross-page continuation merge preserves source groups and dependencies',async()=>{
 ...
 p=mergeQuestionContinuation(p,left.id);
