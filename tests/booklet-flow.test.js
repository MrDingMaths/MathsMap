...
test('Linear conversion preserves all source content and joins explicit continuations',()=>{
 const original=JSON.parse(readFileSync('booklets/projects/linear-relationships-complete-v1.json','utf8')),before=JSON.stringify(original),p=convertToFlexible(original);
...
test('source layout preserves every original page boundary, mixed page and question number',async()=>{
 const source=JSON.parse(readFileSync('booklets/projects/linear-relationships-complete-v1.json','utf8')),p=convertToFlexible(source);
...
test('whole-question arrangements retain intentional empty columns',()=>{
 const source=JSON.parse(readFileSync('booklets/projects/linear-relationships-complete-v1.json','utf8'));
...
test('copying an arranged real question preserves its editable diagrams and validates',()=>{
 const source=JSON.parse(readFileSync('booklets/projects/linear-relationships-complete-v1.json','utf8')),p=convertToFlexible(source),blockId='page-39-q1',destination=p.sections.at(-1).id;
