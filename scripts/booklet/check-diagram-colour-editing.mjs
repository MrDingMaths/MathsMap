...
const out='.booklet-work/diagram-colours';
...
try{
 ...
 await figure.first().click();await page.getByRole('button',{name:'Edit TikZ',exact:true}).click();assert.equal(await dialog.getByLabel('TikZ code',{exact:true}).inputValue(),edited);
 assert.deepEqual(errors,[]);fs.writeFileSync(out+'/editor-qa.json',JSON.stringify({passed:true,saveReopen:true,recoverableInvalidPolicy:true,colours:before},null,2));console.log('Diagram editor, invalid-policy recovery, save and reopen passed');
