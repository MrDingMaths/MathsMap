...
import {observeBookletPerformance} from './performance-observers.mjs';
...
const page=await browser.newPage({viewport:{width:1700,height:1100},...(process.env.BOOKLET_PERF_CACHE?{storageState:process.env.BOOKLET_PERF_CACHE}:{})});page.on('pageerror',e=>errors.push(e.stack));
...
try{
 ...
 const blockId=process.env.BOOKLET_PERF_BLOCK??'index-t1-q1';await page.getByRole('button',{name:'Toggle page navigation'}).click();await page.locator(`.flow-outline [data-block-id="${blockId}"] > button`).click();
 ...
 await page.evaluate(()=>{document.activeElement.position=-1;window.perfField=document.activeElement;window.perfLatex=document.activeElement.value;window.perfInput=[];window.perfPaint=[];window.perfLong=[];window.perfEditor=document.querySelector('maths-editor');window.perfCounts={};for(const name of ['capture','read','remember','emit','render']){const c=window.perfEditor.documentController,original=c[name];c[name]=function(...args){const start=performance.now();try{return original.apply(this,args);}finally{const v=window.perfCounts[name]??={calls:0,ms:0};v.calls++;v.ms+=performance.now()-start;}};}window.perfObserver=new PerformanceObserver(list=>window.perfLong.push(...list.getEntries().map(e=>({start:e.startTime,duration:e.duration}))));window.perfObserver.observe({type:'longtask',buffered:false});document.addEventListener('keydown',e=>{if(!e.target.matches('math-field'))return;const now=performance.now();requestAnimationFrame(()=>{window.perfInput.push(performance.now()-now);requestAnimationFrame(()=>window.perfPaint.push(performance.now()-now));});},true);});
 await page.evaluate(observeBookletPerformance);
 const cdp=await page.context().newCDPSession(page);await cdp.send('Profiler.enable');const profileStart=await page.evaluate(()=>performance.now());await cdp.send('Profiler.start');
 ...
 const measured=await page.evaluate(()=>({session:window.bookletTiming,inputMs:window.perfInput,paintMs:window.perfPaint,longTasks:window.perfLong,counts:window.perfCounts,pageChecks:window.perfPageChecks,latex:window.perfField.value,activeTag:document.activeElement.tagName,editors:[...document.querySelectorAll('maths-editor')].map(e=>({visible:!!e.getClientRects().length,page:e.closest('[data-flow-index]')?.dataset.flowIndex})),fieldRetained:window.perfField.isConnected&&document.activeElement===window.perfField,editorRetained:window.perfEditor===document.querySelector('maths-editor'),pagination:JSON.parse(document.querySelector('.flow-document').dataset.paginationMetrics)}));
 fs.writeFileSync(`${out}/${process.env.BOOKLET_PERF_RUN??'latest'}.cpuprofile`,JSON.stringify(profile));
 const result={project:original.id,cacheInput:process.env.BOOKLET_PERF_CACHE??null,profileStart,openingMs,typingMs,settleMs:Date.now()-started-typingMs,totalMs:Date.now()-started,writes,errors,...measured,hot};fs.writeFileSync(`${out}/${process.env.BOOKLET_PERF_RUN??'latest'}.json`,JSON.stringify(result,null,2));console.log(JSON.stringify(result));assert.deepEqual(errors,[]);
 ...
 await page.locator('.document-toolbar .equation-menu > summary').click();const tools=page.locator('.document-toolbar .me-math-tools');await tools.waitFor();
 ...
 await page.reload();await ready();await page.getByRole('button',{name:'Toggle page navigation'}).click();assert.deepEqual(fieldValue(record,anchor),reopened);
