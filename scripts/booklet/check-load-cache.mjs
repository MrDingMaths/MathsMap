...
try{
 const context=await browser.newContext(),page=await context.newPage(),writes=[],errors=[],bank=[],publicModuleRequests=[];
 page.on('pageerror',e=>errors.push(e.message));
 page.on('request',r=>{if(new URL(r.url()).pathname.startsWith('/public/libs/'))publicModuleRequests.push(r.url());});
 ...
 const sharedEditorModule=await page.evaluate(async()=>{
  const host=await import('/src/lib/document-content.js');await host.loadDocumentEditor();
  const standalone=await import('/libs/maths-editor/document-model.mjs');return host.fromSource===standalone.fromSource;
 });
 assert.equal(sharedEditorModule,true,'Host and standalone editor must share one module instance');
 assert.deepEqual(publicModuleRequests,[],'Browser imports must use /libs, not /public/libs');
