...
\draw (R)--(F)--(L);`;
  ...
  const labelWidthRepairs={'p7-q5-a-front-back':[26,30],'p16-q1-h-diagram':[39.7,43.7],'p16-q2-c-diagram':[40.4,48.4],'p20-q1-k-diagram':[47,51]};
  const widthRepair=labelWidthRepairs[value.id];if(widthRepair&&value.widthMm===widthRepair[0]){value.widthMm=widthRepair[1];records.push({id:value.id,location,before:widthRepair[0],after:widthRepair[1],reason:'Give the complete calibrated 10 pt label room inside its viewport, following the all-question label-bounds audit; geometry and associations unchanged.'});}
  if(value.ref==='p16-q1-h-diagram'&&value.width===39.7){value.width=43.7;records.push({id:value.ref,location,before:39.7,after:43.7,reason:'Reconcile the saved arrangement width with the accepted label-width repair.'});}
  if(value.id==='p20-q1-j-diagram'&&value.widthMm===34.3){value.widthMm=40.3;records.push({id:value.id,location,before:34.3,after:40.3,reason:'Give the complete 10 pt 20.5 cm label enough room inside the native SVG viewport; retain camera, coordinates and label association.'});}
