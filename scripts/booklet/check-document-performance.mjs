...
try{
 ...
 await page.evaluate(()=>{window.perfInput=[];window.perfPaint=[];window.perfLong=[];window.perfEditor=document.querySelector('maths-editor');window.perfObserver=new PerformanceObserver(list=>window.perfLong.push(...list.getEntries().map(e=>({start:e.startTime,duration:e.duration}))));window.perfObserver.observe({type:'longtask',buffered:false});document.addEventListener('beforeinput',()=>{const now=performance.now();requestAnimationFrame(()=>{window.perfInput.push(performance.now()-now);requestAnimationFrame(()=>window.perfPaint.push(performance.now()-now));});},true);});
 ...
 const measured=await page.evaluate(()=>({session:window.bookletTiming,inputMs:window.perfInput,paintMs:window.perfPaint,longTasks:window.perfLong,editorRetained:window.perfEditor===document.querySelector('maths-editor'),pagination:JSON.parse(document.querySelector('.flow-document').dataset.paginationMetrics)}));
