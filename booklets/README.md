...
| [studio-sources/](studio-sources/) | Original PDFs and Word documents for Studio, grouped by stage, then booklet topic. |
...
Place new Studio source documents in `studio-sources/<stage>/<topic>/`. Place new MathsMap source material in `mathsmap-sources/<stage>/`, retaining the relative paths to its media. Keep editable projects in `projects/`; do not place PDF exports in either source collection.
