...
These are source inputs for MathsMap content and skill generation, not editable Booklet Studio projects. Keep their linked `media/` folders with them. Historical batch paths under `booklets/Stage ?` resolve through `scripts/booklet/source-paths.mjs`; the source relocation registry is `booklets/sources.json`.
